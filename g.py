...
def apply_lut_and_watermark(input_video, lut_file, watermark_file, output_video):
    ...
    command = [
        ...
        '-filter_complex', (
            f"[0:v]scale=1080:1920[main];"   # Scale input video to match watermark size if necessary
            f"[main]lut3d='{lut_file}'[v0];" # Apply LUT to the main video
            f"[v0][1:v]overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2"  # Overlay watermark
        ),
